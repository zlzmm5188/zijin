...
try {
    ...
    if ($currency) {
        $where .= ' AND r.currency = :currency';
        $params['currency'] = $currency;
    }

    if ($keyword) {
        $where .= ' AND (u.username LIKE :keyword OR u.phone LIKE :keyword OR r.order_no LIKE :keyword)';
    ...
    $pendingResult = $db->fetchOne(
        "SELECT COUNT(*) as count FROM recharge_records WHERE status = 0",
        []
    );
    ...
    error_log("充值列表API错误: " . $e->getMessage());
    Response::error('获取充值列表失败: ' . $e->getMessage());
}
