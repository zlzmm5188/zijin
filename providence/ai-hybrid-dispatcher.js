...
const AI_HYBRID_DISPATCHER = {
  ...
  apiBase: (typeof AI_CONFIG !== 'undefined' && AI_CONFIG.backend && AI_CONFIG.backend.apiBase) ? AI_CONFIG.backend.apiBase : 'https://apis.copla.top', // 后端API地址
  ...
  async processMessage(userMessage, userData = null) {
    ...
    try {
      ...
      if (AI_CONFIG && AI_CONFIG.backend && AI_CONFIG.backend.enable !== false) {
        try {
          console.log('%c🌐 尝试调用后端API', 'color: #1890ff; font-weight: bold');
...
if (typeof window !== 'undefined') {
  ...
  window.sendToAI = async (message, userData = null) => {
    try {
      console.log('%c[sendToAI] 开始处理', 'color: #1890ff; font-weight: bold', { message, userData });
      ...
      console.log('%c[sendToAI] 处理完成', 'color: #52c41a; font-weight: bold', result);
      return result;
    } catch (error) {
