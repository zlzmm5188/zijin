...
extract_apis_from_file() {
    ...
    # 方法1: 提取 /index.php/ 路径
    grep -oE '/index\.php/[a-zA-Z0-9\/_-]+' "$file" 2>/dev/null | \
    sed 's|/index\.php/||' | \
    sed 's/[?&].*//' | \
    sort -u | \
    grep -v '^$'

    # 方法2: 提取 API_BASE + '/index.php/' 模式
    grep -oE "API_BASE\s*\+\s*['\"]/index\.php/[^'\"]+" "$file" 2>/dev/null | \
    sed "s|.*/index\.php/||" | \
    sed "s/['\"]//g" | \
    sed 's/[?&].*//' | \
    sort -u | \
    ...
    # 方法3: 提取 fetch('/index.php/' 模式
    grep -oE "fetch\(['\"]/index\.php/[^'\"]+" "$file" 2>/dev/null | \
    sed "s|fetch(['\"]/index\.php/||" | \
    sed "s/['\"].*//" | \
    sed 's/[?&].*//' | \
    sort -u | \
    grep -v '^$'
...
test_api() {
    ...
    if [[ "$http_code" == "404" ]]; then
        ALL_ERRORS+=("{\"page\":\"$2\",\"api\":\"$api_path\",\"url\":\"$full_url\",\"error\":\"404 Not Found\",\"code\":null,\"msg\":null}")
    ...
    if echo "$body" | grep -q '"code"'; then
        ...
        if [[ "$code" == "-1" ]]; then
            ALL_ERRORS+=("{\"page\":\"$2\",\"api\":\"$api_path\",\"url\":\"$full_url\",\"error\":\"API返回code:-1\",\"code\":$code,\"msg\":\"$msg\"}")
...
echo "    \"testedPages\": $TESTED_COUNT," >> "$REPORT_FILE"
